...
public class ChannelOsc {
    int id;
    HashMap<Integer, Oscillator> current_notes;    // Pairs <note midi code, oscillator object>
    ...
    ChannelOsc() {
        current_notes = new HashMap<Integer, Oscillator>();
    ...
    void play_note(int note_code, int velocity) {
        ...
        Oscillator s = current_notes.get(note_code);
        if (s == null) {
    ...
    void set_sostenuto(int value) {
        ...
        if (sostenuto_pedal) {
            for (Entry<Integer, Oscillator> s : this.current_notes.entrySet()) {
                if (s.getValue().isPlaying()) {
    ...
    void set_all_oscs_amp() {
        for (Oscillator s : current_notes.values()) {
    ...
    void set_bend(int bits_lsb, int bits_msb) {
        ...
        for (Entry<Integer, Oscillator> s_pair : current_notes.entrySet()) {
